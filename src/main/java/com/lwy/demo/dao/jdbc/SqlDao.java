...
public class SqlDao {
    ...
    public List<String> getTestColumn(String tableName) throws SQLException {
        List<String> columnList = new ArrayList<>();
        try {
            //数据库操作
            Class.forName("com.mysql.cj.jdbc.Driver");
            ...
            //获取字段
            DatabaseMetaData metaData = conn.getMetaData();
            ResultSet colRet = metaData.getColumns("mysqlstudy", "%", tableName, "%");
            while (colRet.next()) {
                String columnName = colRet.getString("COLUMN_NAME");
                columnList.add(columnName);
            }
    ...
    public Object getTest(String sql, List<String> columnList) throws SQLException {
